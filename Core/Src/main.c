...
/**
  * @brief TIM2: 8MHz HSI, Prescaler=7999, Period=999 → tam 1 saniye kesme
  */
static void MX_TIM2_Init(void)
{
    htim2.Instance               = TIM2;
...
/**
  * @brief GPIO: PC13 LED çıkış, PA0 buton girişi (pull-up), PA1 GND referansı
  */
static void MX_GPIO_Init(void)
{
    ...
    // PC13 — Dahili LED (aktif-düşük, başlangıçta kapalı)
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_SET);
    ...
    // PA0 — Buton girişi (dahili pull-up aktif)
    GPIO_InitStruct.Pin  = GPIO_PIN_0;
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

    // PA1 — Lojik 0 referans hattı (h maddesi)
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, GPIO_PIN_RESET);
